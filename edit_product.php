...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $image = $p['image']; // keep existing by default
    ...
    if (!$name || $price <= 0 || $stock < 0) {
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if (!in_array($_FILES['image']['type'], $allowed)) {
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_dir . $filename)) {
                    ...
                    $msg = "Failed to upload image."; $msgtype = 'error';
                }
            }
        }

        // ── Remove image if checkbox ticked ─────────────────
        if (isset($_POST['remove_image']) && !isset($_FILES['image']['name'][0])) {
            if ($p['image'] && file_exists('uploads/products/' . $p['image'])) {
                unlink('uploads/products/' . $p['image']);
            }
            $image = null;
        ...
        if (!$msg) {
            $upd = $conn->prepare("UPDATE products SET name=?, description=?, price=?, stock=?, image=? WHERE id=? AND seller_id=?");
            $upd->bind_param("ssdisii", $name, $desc, $price, $stock, $image, $id, $current_user_id);
            $upd->execute(); $upd->close();
            log_activity($conn, $current_user_id, "edited_product", "$name (ID $id)");
            header("Location: my_products.php?saved=1"); exit();
        }
    }
}

$current_image = $p['image'] ? 'uploads/products/' . $p['image'] : null;
?>
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Edit Product – ShopBlue</title><link rel="stylesheet" href="style.css">
<style>
      ...
      <div class="field">
        ...
        <div class="img-preview-box" onclick="document.getElementById('img-input').click()" id="preview-box">
          <?php if ($current_image && file_exists($current_image)): ?>
            <img src="<?= htmlspecialchars($current_image) ?>?v=<?= time() ?>" alt="product image" id="preview-img">
            <div style="position:absolute;bottom:8px;right:10px;background:rgba(0,0,0,.6);color:#fff;font-size:.75rem;padding:3px 8px;border-radius:4px">Click to change</div>
          <?php else: ?>
        ...
        <?php if ($current_image): ?>
          <label style="display:flex;align-items:center;gap:8px;margin-top:8px;cursor:pointer;flex-direction:row;text-transform:none;font-size:.83rem;color:var(--muted)">
            <input type="checkbox" name="remove_image" value="1" style="width:auto;padding:0;margin:0"> Remove current image
          </label>
        <?php endif; ?>
      </div>

      <div class="field"><label>Product Name</label>
        <input type="text" name="name" required value="<?= htmlspecialchars($_POST['name'] ?? $p['name']) ?>">
      </div>
      <div class="field"><label>Description</label>
        <textarea name="description"><?= htmlspecialchars($_POST['description'] ?? $p['description'] ?? '') ?></textarea>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        <div class="field"><label>Price (&#8369;)</label>
          <input type="number" name="price" step="0.01" min="0.01" required value="<?= htmlspecialchars($_POST['price'] ?? $p['price']) ?>">
        </div>
        <div class="field"><label>Stock Quantity</label>
          <input type="number" name="stock" min="0" required value="<?= htmlspecialchars($_POST['stock'] ?? $p['stock']) ?>">
        </div>
      </div>
      <button type="submit" class="btn btn-full" style="margin-top:4px">Update Product</button>
    </form>
  </div>
</div>
<script>
